...
public class GestorJSONv5Test {
    ...
    public void decodeTele() {
        ...
        try {
            ...
            resultado = this.gestordatos.decodeTele(listaProductos,listaProductos.size()-1);


            assertEquals(this.productoTelevisor.toString(), resultado.toString());


        } catch (IOException ex) {

            fail("No se pudo transformar de JSON a array el AlmacenTelevisor.json");
    ...
    public void decodeCelu() {
        ...
        try {
            ...
            resultado = this.gestordatos.decodeCelu(listaProductos,listaProductos.size()-1);


            assertEquals(this.productoCelular.toString(), resultado.toString());


        } catch (IOException ex) {

            fail("No se pudo transformar de JSON a array el AlmacenCelular.json");
    ...
    public void decodePC() {
        ProductoPC resultado;

        try {
            this.gestordatos.agregarProductoArchivoPC(this.productoPC);
            ArrayList<String> listaProductos = this.gestordatos.vectorLineasPC();
            resultado = this.gestordatos.decodePC(listaProductos,listaProductos.size()-1);


            assertEquals(this.productoPC.toString(), resultado.toString());


        } catch (IOException ex) {

            fail("No se pudo transformar de JSON a array el AlmacenPC.json");
